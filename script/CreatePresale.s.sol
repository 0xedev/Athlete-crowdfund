...
contract CreatePresale is Script {
    ...
    function run() external returns (address presaleAddress) {
        ...
        IERC20Metadata presaleToken = IERC20Metadata(presaleTokenAddress);
        ...
        uint8 decimals = presaleToken.decimals();
        uint8 currencyDecimals = 18;
        if (currencyTokenAddress != address(0)) {
            try IERC20Metadata(currencyTokenAddress).decimals() returns (uint8 dec) {
                currencyDecimals = dec;
            } catch {}
        }
        uint256 tokensNeeded = (hardCap * presaleRate * (10 ** decimals)) / (10 ** currencyDecimals);
